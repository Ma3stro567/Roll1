...
"""
    ...
    async def callback_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Админ функции
            ...
            elif query.data == "admin_ban_list":
    ...
    async def message_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if waiting_for == 'broadcast':
            ...
            try:
                ...
                if target_id in self.users and target_id not in self.banned_users:
                    self.users[target_id].clicks += clicks
                    await update.message.reply_text(f"✅ Пользователю {target_id} начислено {clicks:,} кликов!")
                else:
                    await update.message.reply_text("❌ Пользователь не найден или заблокирован!")
            except:
            ...
            try:
                ...
                if target_id in self.admins:
                    ...
                    try:
                        await context.bot.send_message(target_id, "🚫 Вы были заблокированы в боте администратором!")
                    except:
                        pass
            ...
            try:
                ...
                if target_id in self.banned_users:
                    self.banned_users.remove(target_id)
                    try:
                        await context.bot.send_message(target_id, "✅ Вы были разблокированы! Можете снова пользоваться ботом!")
                    except:
                        pass
